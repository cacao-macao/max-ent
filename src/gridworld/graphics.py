...
from src.gridworld import util
from gridworld.graphics_util import *
...
class GraphicsGridworldDisplay:
    ...
    def displayPolicy(self, agent, currentState=None, message="Agent Policy"):
        policy = {}
        states = self.gridworld.getStates()
        for state in states:
            legal = self.gridworld.getPossibleActions(state)
            legal = list(map(lambda a: agent.env._actToIdx[a], legal))
            probs = agent.policy(agent.env._observe(state), legal=legal)

            action_probs = {}
            for idx, p in enumerate(probs):
                action = agent.env._idxToAct[idx]
                action_probs[action] = p
            policy[state] = action_probs

        drawPolicy(self.gridworld, policy, currentState, message)
...
def drawPolicy(gridworld, policy, currentState=None, message="Policy"):
    grid = gridworld.grid
    blank()
    minValue = 0.0
    maxValue = 1.0
...
def drawSquareP(x, y, probs, minValue, maxValue, isCurrent):
    ...
    for action in actions:
        wedge_color = getColor(probs[action], minValue, maxValue)
